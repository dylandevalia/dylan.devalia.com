...
window.onload = () => {
  ...
  document.getElementById("about-button").onclick = () =>
    showExtra(document.getElementById("about"));
  document.getElementById("contact-button").onclick = () =>
    showExtra(document.getElementById("contact"));
  document.getElementById("cv-button").onclick = () =>
    showExtra(document.getElementById("cv"));
...
function wiggle(a) {
  ...
  const b = getRandom(-0.75, 0.75);
  let c; // = getRandom(-.75, .75);
  if (shift === true) {
    c = getRandom(-0.75 - canvas.height * 0.25, 0.75 - canvas.height * 0.25);
  } else {
    c = getRandom(-0.75, 0.75);
  }
...
function showExtra(a) {
  ...
  name.style.transform = "translateY(-" + canvas.height * 0.25 + "px)";
  shift = true;
  setTimeout(() => {
  ...
  draw() {
    ...
    for (let i = this.i + 1; i < ps.length; i++) {
      ...
      if (dist < 200) {
        context.beginPath();
        context.strokeStyle =
          "rgba(250, 250, 250, " + map(dist, 0, 200, 1, 0) + ")";
